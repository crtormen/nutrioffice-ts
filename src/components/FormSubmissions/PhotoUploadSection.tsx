...
interface PhotoUploadSectionProps {
  ...
  token: string;
  onPhotosChange: (photos: { front?: string; back?: string; side?: string }) => void;
}
...
export function PhotoUploadSection({ positions, token, onPhotosChange }: PhotoUploadSectionProps) {
  const { uploadPhoto, deletePhoto, getPhotoUrl, compressImage } = usePublicFormStorage(token);
  ...
  const handleFileSelect = useCallback(async (position: string, file: File) => {
    try {
      // Update file state
      ...
      const uploadTask = uploadPhoto(compressedFile, position as "front" | "back" | "side");
      ...
      uploadTask.on(
        ...
        async () => {
          ...
          try {
            ...
            setUploading((prev) => ({ ...prev, [position]: false }));
            toast.success(`Foto ${POSITION_LABELS[position]} enviada com sucesso!`);
          } catch (error) {
            console.error(`Error getting download URL for ${position}:`, error);
            toast.error(`Erro ao finalizar envio da foto ${POSITION_LABELS[position]}`);
            setUploading((prev) => ({ ...prev, [position]: false }));
          }
        }
      );
  ...
  const handleRemove = useCallback(async (position: string) => {
    try {
