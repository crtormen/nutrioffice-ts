...
export function MeasurePointsEditor({ measurePoints, onPointsChange }: MeasurePointsEditorProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">Pontos de Medida Circunferencial</CardTitle>
        <CardDescription>Selecione quais medidas devem ser coletadas</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          {sortedMeasurePoints.map((point) => (
            <div key={point.id} className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor={`measure-${point.id}`} className="cursor-pointer text-sm">
                {point.label}
              </Label>
            </div>
          ))}
