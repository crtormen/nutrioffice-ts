...
export function PendingSubmissionsCard() {
  const { dbUid } = useAuth();
  const { data: submissions = [], isLoading } = useFetchFormSubmissionsQuery(
    dbUid || "",
    {
      skip: !dbUid,
    },
            ...
            <div key={submission.id}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {submission.appointmentType === "online"
                      ? "Online"
                      : "Presencial"}
                  </Badge>
                  <Button asChild variant="outline" size="sm">
                    <Link to={ROUTES.FORM_SUBMISSIONS}>Ver detalhes</Link>
                  </Button>
                </div>
              </div>
              {index < Math.min(pendingCount, 3) - 1 && (
